# DataLemur SQL Study

Practice queries from [DataLemur SQL Tutorial](https://datalemur.com/sql-tutorial).

## Progress

### Basic SQL
- [✅] Lesson 101: Tutorial Intro
- [✅] Lesson 102: SQL SELECT
- [✅] Lesson 103: SQL WHERE
- [✅] Lesson 104: AND, OR, NOT
- [✅] Lesson 105: SQL BETWEEN
- [✅] Lesson 106: SQL IN
- [✅] Lesson 107: SQL LIKE
- [✅] Lesson 108: Filtering Review
- [✅] Lesson 109: SQL ORDER BY

### Intermediate SQL
- [✅] Lesson 201: Intermediate SQL Intro
- [✅] Lesson 202: SUM, AVG, COUNT
- [✅] Lesson 203: SQL GROUP BY
- [✅] Lesson 204: SQL HAVING
- [✅] Lesson 205: SQL DISTINCT
- [✅] Lesson 206: SQL Arithmetic
- [✅] Lesson 207: Math Functions
- [✅] Lesson 208: SQL Division
- [✅] Lesson 209: SQL NULL
- [ ] Lesson 210: SQL CASE
- [ ] Lesson 211: SQL JOINs
- [ ] Lesson 212: Date Functions

### Advanced SQL
- [ ] Lesson 301: Advanced SQL Intro
- [ ] Lesson 302: CTE vs Subquery
- [ ] Lesson 303: Window Functions
- [ ] Lesson 304: SQL Ranking
- [ ] Lesson 305: LEAD and LAG
- [ ] Lesson 306: Self-Joins
- [ ] Lesson 307: SQL UNION
- [ ] Lesson 308: Write Clean SQL
- [ ] Lesson 309: Execution Order
- [ ] Lesson 310: SQL Pivoting
- [ ] Lesson 311: String Functions
- [ ] Lesson 312: Instacart Case Study

## Structure

Each `.sql` file contains practice queries and notes for that lesson.

Files are named: `lesson-101-intro.sql`, `lesson-102-select.sql`, etc.
